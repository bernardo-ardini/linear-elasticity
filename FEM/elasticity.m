classdef elasticity < handle
    ...
    methods
        ...
        function [w,res,iter]=solve(obj,precond)
            ...
            [obj.u,res,iter]=PCG(B,Phi,zeros(size(B,2),1),1e-8,1e6,L); % soluzione del sistema lineare con PCG
                ...
                for e=1:size(obj.topol,1)
                    if ismember(a,obj.topol(e,:))
                        ...
                        for b=1:4
                            ...
                            t=t+Delta*obj.u(3*(obj.topol(e,b)-1)+i)*obj.C(H);
                        end
                    end
                end
            end
        ...
        function s=sigma(obj)
            s=zeros(size(obj.coord,1),1);

            for a=1:size(obj.coord,1)
                s(a)=norm(obj.T(a),"fro");
            end
        end
